...
impl AudioPlayer {
    ...
    /// Atomically dump mpv's current demuxer cache to a file via the
    /// libmpv `dump-cache` command. Wraps the trait method so callers
    /// outside the playback crate (the prefetch worker) don't need to
    /// hold an `Arc<dyn MpvPlayer>` directly.
    pub fn dump_cache_to(&self, path: &str) {
        self.mpv.dump_cache_to(path);
    }
