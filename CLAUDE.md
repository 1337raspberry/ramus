...
- **Search operator syntax** (`/genre`, `@artist`, `!track`, `%album`, `#>2000` or `year:>2000`, `fav:`) segments on `" AND "` (case-sensitive uppercase). Without AND, entire input belongs to the first operator. Typing an operator key (`/`, `@`, `%`, `!`, `#`) outside an input field opens the search overlay with that character pre-loaded. Shift+Enter in the search overlay loads all matching albums into the grid for browsing.
